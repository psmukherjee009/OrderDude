...
var global = {
	GetOrderUpdate_Url : '/cgi-bin/VirtualOrdering_3.cgi/GETORDERUPDATE/',
	UpdateOrder_Url : '/cgi-bin/VirtualOrdering_3.cgi/UPDATEORDER/',
	SendNewOrder_Url : '/cgi-bin/VirtualOrdering_3.cgi/SENDNEWORDER/',
	Get_Restaurant_List_Url : '/cgi-bin/VirtualOrdering_3.cgi/GETINFO/',
	Get_Business_Details_Url : '/cgi-bin/VirtualOrdering_3.cgi/GETBUSINESS/',
...
function ops_addItem(urlObj, options) {
	var qstr = parseUrlforinput(urlObj.href);
...
function get_OrderTable() {
	...
	global.sRestaurant.Cart.GrandTotal = Math.round((global.sRestaurant.Cart.Total+tax_amount+tip_amount)*100)/100;
	...
	markup += '<tr><td colspan=3>----------------------------------</td></tr>';
	markup += '<tr><td colspan=3>----------------------------------</td></tr>';
	markup += '<tr><td>Tip ('+global.sRestaurant.Cart.Tip+' %)</td><td></td><td>'+tip_amount+'</td></tr>';
	markup += '<tr><td colspan=3>----------------------------------</td></tr>';
...
function Page_Cart() {
	...
	if ( localStorage.getItem('Name') && localStorage.getItem('Phone') ) { // Have the order inputs so go to orders directly
		markup += '<li><a href="#sendnewdineinorder" data-role="button">Dine In</a></li>';
		markup += '<li><a href="#sendnewpickuporder" data-role="button">Pick Up</a></li>';
		//markup += '<a href="#sendneworder" data-role="button">DineIn</a>';
		//markup += '<a href="#sendneworder" data-role="button">CarryOut</a>';
		
	}
	else {
		markup += '<li><a href="#collectdineininfo" data-role="button" data-rel="dialog" data-transition="pop" data-rel="back">Dine In</a></li>';
		markup += '<li><a href="#collectpickupinfo" data-role="button" data-rel="dialog" data-transition="pop" data-rel="back">Pick Up</a></li>';
		//markup += '<a href="#collectinfo" data-role="button" data-rel="dialog" data-transition="pop" data-rel="back">Order</a>';
	}
	markup += '</ul>';
	//markup += '</div>';
	// Inject the category items markup into the content element.
...
function ops_CollectDineInInfo(urlObj, options) {
	Page_CollectInfo(urlObj, options,'#sendnewdineinorder','Dine In');
}

function ops_CollectPickUpInfo(urlObj, options) {
	Page_CollectInfo(urlObj, options,'#sendnewpickuporder','Pick Up');
}
...
function SendNewOrder(message) {
	// Transmit the order
	var order_url = url_base() + global.SendNewOrder_Url+global.Input.Phone+'/'+global.sRestaurant.Info.id+'/';
...
function ops_sendNewDineInOrder(urlObj, options) {
	global.Cart.OrderType = 'DINEIN';
	sendNewOrder(urlObj, options);
}

function ops_sendNewPickUpOrder(urlObj, options) {
	global.Cart.OrderType = 'PICKUP';
	sendNewOrder(urlObj, options);
}

function ops_sendNewOrder(urlObj, options,order_type) {
	var qstr = parseUrlforinput(urlObj.href);
	global.Cart.OrderType = 'PICKUP';
	if ( qstr['submit'] === 'DineIn' ) {
		global.Cart.OrderType = 'DINEIN';
	}
	sendNewOrder(urlObj, options);
}

function sendNewOrder(urlObj, options,order_type) {
...
function ops_getMessage() {
	...
	// Transmit the order
	var get_orderupdate_URL = url_base() + global.GetOrderUpdate_Url+global.Input.Phone+'/'+global.sRestaurant.Info.id+'/'+global.Cart.OrderId+'/?lastupdatetime='+global.Cart.LastUpdateTime;
...
function Page_Order() {
	...
	if ( global.Cart.OrderType == 'DINEIN' ) {
...
function ops_Restaurant(url) {
	var qstr = parseUrlforinput(url);
	if ( typeof qstr['id'] === 'undefined' ) {
		return ops_Home(1);
	}
	if ( (global.sRestaurant != null) && (global.sRestaurant.Info.id != qstr['id']) && (global.sRestaurant.Cart.NoItems > 0) ) {
	...
	if ( (global.sRestaurant == null)  || (global.sRestaurant.Info.id != global.Input['id']) ) {
		global.sRestaurant = new Object();
		global.sRestaurant.Cart = global.Cart;
		var bURL = url_base() + global.Get_Business_Details_Url+global.Input['id']+'/';
...
$(document).ready(function() {
	...
	if ( typeof q['id'] != 'undefined' ) { // Asking for Restaurant Page
