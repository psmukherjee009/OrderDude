...
var global = {
	...
	ReOrderLink : 'http://www.orderdude.com/index_5.html?reorder_id=',
	...
	if ( (typeof po_list != 'undefined') && (po_list != null) ) {
		...
		if ( po_array.length != 0 ) {
			...
			for ( var i = 0 ; i < po_array.length ; ++i ) {
				...
				markup += '<li data-icon="false"><a href="'+global.ReOrderLink+i+'" rel="external">'+txt+'</a><a href="#delete_order?id='+i+'"></li>';
			}
			markup += "</ul>";
		}
		else {
...
function getMyLocation() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(displayLocation,displayAddressInput);
    } else {
        displayAddressInput();
    }
}
...
function displayLocation(position) {
	console.log("Position Co-ordinates = "+JSON.stringify(position));
...
function getOrderPersonalizationItemDisplayBlock(cName_escaped,key,key_escaped,priceObj,elements) {
	...
	for ( var i = 0 ; i < elements.order ; ++i ) {
		...
		markup += '<a href="#personalize?category='+cName_escaped+'&item='+key_escaped+'&price='+priceObj+'&number='+(i+1)+'" rel="external" >Select for order '+(i+1);
...
function OrderConfirmCountdown() {
	if ( countdown > 0 ) {
		$('#CalltoConfirmOrder').html("Call to Confirm Order in "+countdown+" secs");
	}
	else {
...
function Page_CompleteOrder() {
	updateClientOrderDisplay();
	var str_order_complete_header = "Order Sent Successfully";
	...
	if ( typeof global.sRestaurant.Info.order_confirm != 'undefined' ) {
		str_order_complete_header = "Order Faxed Successfully";
		messageBox = '<h2>Your Order is faxed'+global.Message+'</h2>';
		confirmMsg = '<a id="CalltoConfirmOrder" data-role="button" data-icon="phone" href="tel:'+global.sRestaurant.Info.order_confirm+'">Call to Confirm Order</a>';
