...
function buildCategoryBlock(MenuObj,menuname) {
	...
	Object.keys(MenuObj).forEach(function(category) {
		...
		if ( typeof(MenuObj.category.price === undefined ) {
			// This menu section should provide a collapsible menu
			menustr += '<div data-role="collapsible" data-theme="c" data-content-theme="d">';
			menustr += '<h3><ul data-role="listview"data-inset="true" data-theme="b"><li>'+menuname+'</li></ul></h3>';
			menustr += buildCategoryBlock(MenuObj.category,categoryname);
			menustr += "</div>";
		}
		else {
			
			categoryitems += '<li><a href="#menuitems?item='+categoryname+'">'+categoryname+'</a></li>';
		}
	});
	if ( categoryitems !== "" ) {
		categorystr = "<ul data-role='listview' data-inset='true'>".categorystr;
		categorystr += "</ul>";
		return categorystr;
	}
	return menustr;
