var global = {
	id : -1,
	HOME_Url : 'http://www.orderdude.com/VirtualOrdering.html',
	Base_Url : '',
...
function redirect_2_home() {
	window.location = global.HOME_Url;
}
...
function Page_Cart() {
	...
	// Tip Slider
	//var tipslider = '<input type="range" name="slider-tip" id="slider-tip" value="0" min="0" max="30" data-highlight="true"/>';
	
	//$('#slider-tip').change(function() {
	//	alert('Value is ' + $(this).val() );
	//});
	//$footer.find("h4").html('Tips : '+tipslider);
	$page.page();
	//options.dataUrl = urlObj.href;
	//$.mobile.changePage( $page, options );
...
function ops_Restaurant() {
	if ( (global.sRestaurant != null) && (global.sRestaurant.Info.id != global.id) && (global.sRestaurant.Cart.NoItems > 0) ) {
		//alert(JSON.stringify(global.sRestaurant));
		if ( confirm(global.sRestaurant.Info.name+' order cart will be discarded ') != true ) {
			//alert('Redirecting from 2');
			return redirect_2_home();
		}
		ops_clearOrder();
	}
	global.sRestaurant = new Object();
	global.sRestaurant.Cart = global.Cart;
	var bURL = global.Get_Business_Details_Url+global.id+'/';
...
$(document).ready(function() {
	...
	//alert(global.id);
	if ( (typeof q['id'] != 'undefined') || (global.id != -1) ) { // Asking for Restaurant Page
		var urlObj = $.mobile.path.parseUrl(window.location.href);
		if ( global.id == -1 ) {
			global.id = q['id'];
		}
		global.Base_Url = urlObj.hrefNoSearch;
		ops_Restaurant();
	}
	else {
		//alert('Redirecting from 1');
		redirect_2_home();
	}
});