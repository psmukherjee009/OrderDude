...
function Page_Cart(urlObj, options) {
	...
	$content = $page.children( ":jqmData(role=content)" );
	$footer = $page.children( ":jqmData(role=footer)" );
	$header.find( "h1" ).html('Cart <sup><span class="ui-li-count">'+global.sRestaurant.Cart.NoItems+'</span></sup>');
...
function Page_Order() {
	...
	var messageBox = '<h2>Pickup your order using Order # '+global.sRestaurant.Cart.OrderId+'</h2>';
