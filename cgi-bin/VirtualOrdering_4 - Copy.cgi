...
#Version : 4.0
#Capability : ['Accept Payment','Streamedlined GET and POST input']
...
sub getPPPayKey {
	...
	$cmd .= '-H "X-PAYPAL-SECURITY-USERID: pp_1353421194_biz_api1.orderdude.com" ';
	$cmd .= '-H "X-PAYPAL-SECURITY-PASSWORD: 1353421212" ';
	$cmd .= '-H "X-PAYPAL-SECURITY-SIGNATURE: AVnLV.sPA0EdyGiyjYdBJQrL1OPwAyXUfkLAS4-D3ncVofUxxhsbz5ER" ';
	$cmd .= '-H "X-PAYPAL-REQUEST-DATA-FORMAT: JSON" ';
	$cmd .= '-H "X-PAYPAL-RESPONSE-DATA-FORMAT: JSON" ';
	$cmd .= '-H "X-PAYPAL-APPLICATION-ID: APP-1JE4291016473214C" ';
	$cmd .= '-X POST ';
	$cmd .= 'https://svcs.sandbox.paypal.com/AdaptivePayments/Pay ';
	$cmd .= '--data \'';
	
	$cmd .= '{"requestEnvelope":{"errorLanguage":"en_US"},"currencyCode":"USD","actionType":"PAY",';
	$cmd .= '"receiverList":{"receiver":[{"amount":"'.$grand_total.'","email":"pp@orderdude.com"}]},';
	$cmd .= '"cancelUrl":"https://www.orderdude.com#completeorder",';
	$cmd .= '"returnUrl":"https://www.orderdude.com#completeorder?pid='.$payment_track_id.'",';
	$cmd .= '"receiverOptions":[';
	$cmd .= 	'{"receiver":{"email":"pp@orderdude.com"},';
	$cmd .= 		'"customId": "OrderDude Merchant ID",';
	$cmd .= 		'"description":"OrderDude Inc.",';
	$cmd .= 		'"invoiceData":'.$invoiceData.''';
	$cmd .=		'}';
	$cmd .=		']';
	$cmd .= '}';
	...
	my $ret_msg = `$cmd`;
	my $json = JSON->new->allow_nonref;
	$pp_hash = $json->pretty->decode($cmd);
...
sub CompleteOrder() {
	...
	if ($data{'pid'}) { #Payment is done so validate it
		my $cmd = "select enc_order_id from $table_order where id = ".$cmd[2];
		my @data = get_from_DB($cmd);
		if ( ($#data > 0) && ($data[0] eq $data{'pid'}) ) {
			my $cmd = "update $table_order set status = 3,Payment=\"Paid\" where id = ".$cmd[2];
			execute_on_DB($cmd);
			$ret_msg{'Payment'} = 1;
		}
	}
