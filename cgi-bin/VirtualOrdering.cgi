...
sub createOrderDisplay {
    ...
    while (my ($key,$val) = each (%{$order_hash})) {
        ...
        foreach my $item (@{$val}) {
			...
			my $no_tabs = (120-length($name))/12;
            my $item_text = "\n$name";
			for ( my $i = 0 ; $i < $no_tabs ; ++$i ) {
				$item_text .= "\t";
			}
			$item_text .="$no_tabs ".length($name)." x $quantity\t\t\t$price\n";
...
sub createpdffile {
	...
	if (! -e $filename ) {
		...
		my $fax_order_page = "GENERALFAXORDER";
		if ( $name eq "Chipotle" ) {
			$fax_order_page = "CHIPOTLEFAXORDER2";
		}
		
		my $cmd = "./wkhtmltopdf-i386 -q -O landscape http://www.orderdude.com/cgi-perl/VirtualOrdering.cgi/$fax_order_page\/$order_id\/  $filename";
...
sub SendFax {
	my ($order_id,$fax,$is_live,$name) = @_;
	
	# Create the order file
	#my $filename = createhtmlfile($order_id);
	my $filename = createpdffile($order_id,$name);
...
sub print_chipotle_fax_outline2 {
	...
	my $address_line1 = substr $address,0,-6;
...
sub createFaxOrderDisplay1 {
    ...
    while (my ($key,$val) = each (%{$order_hash})) {
        ...
        foreach my $item (@{$val}) {
			...
			my $no_space = (190-2*length($name));
            my $item_text = "\n$name ".length($name);
			for ( my $i = 0 ; $i < $no_space ; ++$i ) {
				$item_text .= "\&nbsp;";
			}
			$item_text .= " x $quantity\t\t\t$price\n";
			for ( my $i = 0 ; $i <= $#personalization; ++$i ) {
				$item_text .= "\tPersonalization #".($i+1);
				if ( $personalized_price[$i] ) {
					$item_text .= "\t\$".$personalized_price[$i]."\n";
...
sub createFaxOrderDisplay {
    ...
    while (my ($key,$val) = each (%{$order_hash})) {
        ...
        foreach my $item (@{$val}) {
            ...
            my $item_text = "<tr><td colspan=\"2\">$name</td>";
			$item_text .= "<td> x $quantity</td></tr>";
			for ( my $i = 0 ; $i <= $#personalization; ++$i ) {
				#$item_text .= "<tr><td></td><td>Personalization #".($i+1)."</td>";
				$item_text .= "<tr>";
				my $pp = "";
				if ( $personalized_price[$i] ) {
					$pp = "<td>\$".$personalized_price[$i]."</td>";
				}
				else {
					$pp = "";
				}
...
sub viewableFaxOrder {
	my ($order_id,$hash) = @_;
	
	
	# Create the Order display first
	my $txt = "<table id='fax_table'>".createFaxOrderDisplay($hash->{'order'});
	#$txt .= "<tr><td colspan=\"4\">----------------</td></tr>";
	#$txt .= "<tr><td>Total</td><td></td><td></td><td>".($hash->{'Total'})."</td></tr>";
	#$txt .= "<tr><td colspan=\"4\">----------------</td></tr>";
	#$txt .= "<tr><td>Tax<td></td><td></td><td>".($hash->{'Tax'})."</td></tr>";
	#$txt .= "<tr><td colspan=\"4\">----------------\n";
	#$txt .= "<tr><td>Total after Tax<td></td><td></td><td>".($hash->{'Total after Tax'})."</td></tr>";
	#$txt .= "<tr><td colspan=\"4\">----------------</td></tr>";
	#$txt .= "<tr><td>Tips<td></td><td></td><td>".($hash->{'Tip'})."</td></tr>";
	#$txt .= "<tr><td colspan=\"4\">----------------</td></tr>";
	#$txt .= "<tr><td>Grand Total<td></td><td></td><td>".($hash->{'Grand Total'})."</td></tr>";
	#$txt .= "<tr><td colspan=\"4\">----------------</td></tr>";
	#$txt .= "<tr><td colspan=\"4\">Name = ".$hash->{'Name'}."</td></tr>";
	#$txt .= "<tr><td colspan=\"4\">Phone = ".$hash->{'Phone'}."</td></tr>";
	#$txt .= "<tr><td colspan=\"4\">Payment = ".$hash->{'Payment'}."</td></tr>";
	#$txt .= "<tr><td colspan=\"4\">Order # OD-$order_id</td></tr>";
	$txt .= "</table>";
	return $txt;
}

sub print_general_fax_line {
	my ($i,$price,$type,$po,$po_amt,$name,$quantity) = @_;
	...
	for ( my $j = 0 ; $j < $quantity; ++$j ) {
		...
		my $top_pix = 200+($fax_line_no*30.8);
		#$i,$price,$type,$po,$po_amt,$name,$quantity
		print "<div style=\"position:absolute;left:34px;top:".$top_pix."px\" class=\"cls_005\"><span class=\"cls_005\">".($fax_line_no+1)."</span></div>";
		print "<div style=\"position:absolute;left:48.27px;top:".$top_pix."px\" class=\"cls_010\"><span class=\"cls_010\">$name</span></div>";
		print "<div style=\"position:absolute;left:180.12px;top:".$top_pix."px\" class=\"cls_010\"><span class=\"cls_010\">$i</span></div>";
		print "<div style=\"position:absolute;left:268.32px;top:".$top_pix."px\" class=\"cls_006\"><span class=\"cls_006\">$price</span></div>";
		print "<div style=\"position:absolute;left:288.73px;top:".$top_pix."px\" class=\"cls_006\"><span class=\"cls_006\">$type</span></div>";
		print "<div style=\"position:absolute;left:313.26px;top:".$top_pix."px\" class=\"cls_006\"><span class=\"cls_006\">".(@$po)."</span></div>";
		print "<div style=\"position:absolute;left:345.36px;top:".$top_pix."px\" class=\"cls_006\"><span class=\"cls_006\">$po_amt</span></div>";
		print "<div style=\"position:absolute;left:370.20px;top:".$top_pix."px\" class=\"cls_006\"><span class=\"cls_006\">$quantity</span></div>";
		#print "<div style=\"position:absolute;left:393.27px;top:".$top_pix."px\" class=\"cls_006\"><span class=\"cls_006\">".(($ch eq "CA")?"CA":"")."</span></div>";
		#print "<div style=\"position:absolute;left:416.57px;top:".$top_pix."px\" class=\"cls_006\"><span class=\"cls_006\">".(($ch eq "S")?"S":"")."</span></div>";
		#print "<div style=\"position:absolute;left:435.27px;top:".$top_pix."px\" class=\"cls_006\"><span class=\"cls_006\">".(($ch eq "B")?"B":"")."</span></div>";
		#print "<div style=\"position:absolute;left:453.40px;top:".$top_pix."px\" class=\"cls_006\"><span class=\"cls_006\">".(($ch eq "V")?"V":"")."</span></div>";
...
<div style="position:absolute;left:50%;margin-left:-420px;top:0px;width:841px;height:595px;border-style:outset;overflow:hidden">
<div style="position:absolute;left:0px;top:0px">
<img src="http://www.orderdude.com/images/GeneralFaxLayout.png" width=841 height=595></div>
<div style="position:absolute;left:227.64px;top:12.92px" class="cls_026"><span class="cls_026">Fax | $address_line1 </span></div>
<div style="position:absolute;left:175.44px;top:58.86px" class="cls_002"><span class="cls_002">DATE</span></div>
<div style="position:absolute;left:233.45px;top:58.86px" class="cls_002"><span class="cls_002">COMPANY NAME</span></div>
<div style="position:absolute;left:390.44px;top:58.86px" class="cls_002"><span class="cls_002">CONTACT NAME</span></div>
<div style="position:absolute;left:545.44px;top:58.86px" class="cls_002"><span class="cls_002">PHONE</span></div>
...
sub GeneralFaxOrder {
	...
	if ( $#data >= 0 ) {
		...
		print_general_fax_outline($res_id);
		...
		#			}
		#			print_general_fax_line($i,$price,$type,$po,$po_amt,$name,$quantity);
		#		}
		#	}
		#}
		my $order_text = viewableFaxOrder($order_id,$order_hash);
		$order_text =~ s/\n/<br \/>/g;
		$order_text =~ s/\t/\&nbsp;\&nbsp;\&nbsp;\&nbsp;\&nbsp;\&nbsp;\&nbsp;\&nbsp;\&nbsp;\&nbsp;\&nbsp;\&nbsp;/g;
		...
		#my $top_pix = 200+($fax_line_no*30.8);
		print "<div style=\"position:absolute;left:48.27px;top:140px\" class=\"cls_006\"><span class=\"cls_006\">$order_text</span></div>";
		print "</div></body></html>";
		exit();
	}
	else {
...
<div style="position:absolute;left:227.64px;top:12.92px" class="cls_026"><span class="cls_026">Fax Order Form | $address </span></div>
...
#my $dup_cmd;
# Check Duplicate Order and in that case just send back the earlier order num
sub Is_duplicate {
	my ($from,$to,$total,$name,$phone) = @_;
	
	my $cmd = "select id
				from $table_order ob
				where (NOW() - ob.create_time) < 60
					and ob.from = $from
					and ob.to = $to
					and ob.total = $total
					and ob.name = '$name'
...
sub SendNewOrder() {
	...
	# Check for duplicate
	my $order_id = Is_duplicate($cmd[2],$cmd[3],$order_hash->{"Grand Total"},$order_hash->{"Name"},$order_hash->{"Phone"});
	my $dup_order = ( $order_id == -1 ) ? 0 : 1;
	if ( $order_id == -1 ) {
		# Create the Order
		$order_id = gen_new_order_id();
	...
	if ( ($#cmd >= 3) && ($cmd[4] eq 'PAY2') ) {
		...
		my $cmd = "update $table_order set enc_order_id = \"$payment_track_id\" where id = $order_id";
		execute_on_DB($cmd);
	...
	#$ret_hash{"DUP_CMD"} = $dup_cmd;
	#$ret_hash{"CMD"} = $cmd;
	$ret_hash{"is_dup"} = $dup_order;
	returnJSONData(\%ret_hash);
}
...
sub Main() {
	...
	CMD qr{^/GENERALFAXORDER/.*$}	=>	\&GeneralFaxOrder;
